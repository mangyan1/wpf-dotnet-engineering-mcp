...
NuGet versions are centralized in `Directory.Packages.props`, and every project has a committed lock file. Dependabot opens weekly update pull requests for the optional VS Code extension manifest and GitHub Actions; NuGet updates are deliberately not automated because Dependabot's lock-file regeneration strips the `win-x64` runtime sections our locked restore requires, so NuGet versions are bumped by hand: edit `Directory.Packages.props`, run `dotnet restore DotNetEngineeringMcp.sln --runtime win-x64` to regenerate the lock files, and verify with `dotnet restore DotNetEngineeringMcp.sln --runtime win-x64 --locked-mode`. Pull requests must pass locked restore, the NuGet advisory gate, dependency review, build, tests, static checks, and the sanitized secret scan before merge.
