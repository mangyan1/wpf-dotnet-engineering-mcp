...
namespace EngineeringMcp.Wpf.WpfUi;

public interface IWpfUiInspectionService
{
    Task<ToolResult<object>> GetResourceAsync(int processId, string automationId, string resourceKey, CancellationToken cancellationToken = default);
    Task<ToolResult<object>> GetPropertyAsync(int processId, string automationId, string property, CancellationToken cancellationToken = default);
    Task<ToolResult<object>> GetThemeEvidenceAsync(int processId, CancellationToken cancellationToken = default);
}

public sealed class WpfUiInspectionService(IWpfProbeClient probe) : IWpfUiInspectionService
{
