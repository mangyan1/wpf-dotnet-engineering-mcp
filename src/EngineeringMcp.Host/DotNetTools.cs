...
public static class DotNetTools
{
    ...
    public static ToolResult<RuntimeProcessInfo> RuntimeInfo([Description("Operating-system process identifier of an allowlisted target process.")] int processId, DotNetDiagnosticsService diagnostics, ToolAuthorization auth)
        => ToolRun.Sync(auth, ToolPolicyCatalog.Get("dotnet_runtime_info").ToPolicy(), processId.ToString(), () => diagnostics.GetRuntimeInfo(processId));
    ...
    public static Task<ToolResult<IReadOnlyList<RuntimeCounterObservation>>> Counters(
        ...
        => ToolRun.Async(auth, ToolPolicyCatalog.Get("dotnet_counters").ToPolicy(), processId.ToString(),
            () => diagnostics.CaptureCountersAsync(processId, durationMs, cancellationToken), cancellationToken);
    ...
    public static Task<ToolResult<IReadOnlyList<RuntimeCounterObservation>>> GcSummary(
        ...
        CancellationToken cancellationToken,
        [Description("Bounded capture duration in milliseconds; the server applies a hard upper bound.")] int durationMs = 10_000)
        => ToolRun.Async(auth, ToolPolicyCatalog.Get("dotnet_gc_summary").ToPolicy(), processId.ToString(), async () =>
        {
    ...
    public static ToolResult<IReadOnlyList<ProcessThreadObservation>> Threads(
        ...
        => ToolRun.Sync(auth, ToolPolicyCatalog.Get("dotnet_threads").ToPolicy(), processId.ToString(), () => diagnostics.GetThreads(processId, maxThreads));
    ...
    public static ToolResult<IReadOnlyList<ProcessModuleObservation>> Modules(
        ...
        => ToolRun.Sync(auth, ToolPolicyCatalog.Get("dotnet_modules").ToPolicy(), processId.ToString(), () => diagnostics.GetModules(processId, maxModules));
    ...
    public static Task<ToolResult<IReadOnlyList<ExceptionObservation>>> Exceptions(
        ...
        => ToolRun.Async(auth, ToolPolicyCatalog.Get("dotnet_exceptions").ToPolicy(), processId.ToString(),
            () => diagnostics.CaptureExceptionsAsync(processId, durationMs, cancellationToken), cancellationToken);
    ...
    public static Task<ToolResult<TraceHandle>> TraceStop(
        ...
        ToolAuthorization auth,
        CancellationToken cancellationToken)
        => ToolRun.Async(auth, ToolPolicyCatalog.Get("dotnet_trace_stop").ToPolicy(), traceId, () => diagnostics.StopTraceAsync(traceId, cancellationToken), cancellationToken);
    ...
    public static Task<ToolResult<object>> CaptureDump([Description("Operating-system process identifier of an allowlisted target process.")] int processId, ClrMdService clrmd, ToolAuthorization auth, CancellationToken cancellationToken)
        => ToolRun.Async(auth, ToolPolicyCatalog.Get("dotnet_capture_dump").ToPolicy(), processId.ToString(), () => clrmd.CaptureDumpAsync(processId, cancellationToken: cancellationToken), cancellationToken);
    ...
    public static ToolResult<DumpAnalysisSummary> AnalyzeDump(
        ...
        => ToolRun.Sync(auth, ToolPolicyCatalog.Get("dotnet_analyze_dump").ToPolicy(), dumpId, () => clrmd.AnalyzeCapturedDump(dumpId, maxThreads, maxFramesPerThread));
}
