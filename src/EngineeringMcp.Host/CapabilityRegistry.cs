...
{
    // Policy is immutable after startup (restart-required model), so availability is built once,
    // lazily on first use, instead of rebuilding a 20-entry dictionary plus env-var reads on
    // every authorize call. A benign duplicate build is possible on the first concurrent calls.
    private IReadOnlyDictionary<string, bool>? _availability;

    private IReadOnlyDictionary<string, bool> Current => _availability ??= BuildAvailability();

    private IReadOnlyDictionary<string, bool> BuildAvailability()
    {
    ...
    private static bool HasStrongToken(string name)
        => Environment.GetEnvironmentVariable(name) is { Length: >= McpRuntimeDefaults.MinimumTokenLength } value
