...
namespace EngineeringMcp.AspNetCore;

public interface IBackendProbeClient
{
    Task<ToolResult<BackendProbeResponse>> RequestAsync(int processId, string operation, int limit = 100, CancellationToken cancellationToken = default);
}

public sealed class BackendProbeClient(IProcessGuard processGuard, IRedactionService redactor, IPolicyProvider policyProvider) : IBackendProbeClient
{
